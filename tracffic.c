...
{
    int code, starttime[2], endtime[2], i, count = 0;
    float money;
    char startcity[10], endcity[10], flag = 'y';
    FILE *fp;
    // 输入所有的航班信息 
    while(flag == 'Y' || flag == 'y')
    {
        ...
        //将每一个航班信息存放在结构体a中 
        a[count].co = code; 
        strcpy(a[count].vt, startcity);
        strcpy(a[count].vh, endcity);
        a[count].bt[0] = starttime[0];
        a[count].bt[1] = starttime[1];
        a[count].at[0] = endtime[0];
        a[count].at[1] = endtime[1];
        a[count].mo=money;
        count++; 
        printf("继续输入?(Y/N)");
        scanf("%c",&flag);
            ...
            case 2:  //展示列车信息 
                ...
                for(j = 0; j < G->vexnum; j++)
                {
                    ...
                    while(q != NULL)
                    {
                        printf("%s---->%s\n", G->vertices[j].cityname, G->vertices[q->adjvex].cityname);
                        for(k = 0; k <= q->info.last; k++)
                            printf("    车次编号:%d  费用:%5.2f 发车时间:%5.2f  到达时间:%5.2f\n", q->info.stata[k].number, q->info.stata[k].expenditure, q->info.stata[k].begintime, q->info.stata[k].arrivetime);
                        q = q->nextarc;
                    }
                }
                break;
